...
public class upload_videos extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wrappersReference = FirebaseDatabase.getInstance().getReference("videos");

        titleEditText = findViewById(R.id.titleEditText);
        descriptionEditText = findViewById(R.id.youtubeLinkEditText);

        wrapperImageView = findViewById(R.id.imageView);
        ...
        nav_view.setNavigationItemSelectedListener(new NavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
                ...
                if (itemId == R.id.dashboard) {
                    Intent intent = new Intent(upload_videos.this, Admin_home_page.class);
                    startActivity(intent);
                } else if (itemId == R.id.Add_Enterprises) {
                    // Redirect to GstCalculationActivity when "Dashboard" is clicked
                    Intent intent = new Intent(upload_videos.this, add_enterprise_list.class);
                    startActivity(intent);
                } else if (itemId == R.id.Add_News) {
                    Intent viewWrappersIntent = new Intent(upload_videos.this, AddNews.class);

                    startActivity(viewWrappersIntent);

                } else if (itemId == R.id.Add_Videos) {
                    Intent viewWrappersIntent = new Intent(upload_videos.this, upload_videos.class);
...
//                    startActivity(viewWrappersIntent);
                    ...
                    Intent viewWrappersIntent = new Intent(upload_videos.this, admin_login.class);
...
//                    startActivity(viewWrappersIntent);
    ...
    private void uploadWrapper() {
        ...
        String youtubeLink = descriptionEditText.getText().toString();

        if (imageUri != null) {
            StorageReference fileReference = storageReference.child("videos").child(title + System.currentTimeMillis() + ".jpg");

            fileReference.putFile(imageUri)
                    .addOnSuccessListener(taskSnapshot -> {
                        fileReference.getDownloadUrl().addOnSuccessListener(uri -> {
                            // Create a new News object with title, description, and imageUrl
                            videos video = new videos(title, youtubeLink, uri.toString());

                            // Add the news to the database under the title
                            DatabaseReference newWrapperRef = wrappersReference.child(title);
                            newWrapperRef.setValue(video);

                            Toast.makeText(upload_videos.this, "video added successfully", Toast.LENGTH_SHORT).show();
                            finish();
                        });
                    })
                    .addOnFailureListener(e -> {
                        Toast.makeText(upload_videos.this, "Upload failed: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    });
        } else {
